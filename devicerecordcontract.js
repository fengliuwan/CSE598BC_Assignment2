...
class DeviceRecordContract extends Contract {
  ...
  async afterTransaction(ctx, result) {
    ...
    console.log(func_and_params);
    console.log(
      func_and_params["fcn"] === "createDeviceRecord" &&
    ...
    if (
      ...
      ctx.stub.setEvent(
        "rare-device-type",
        JSON.stringify({ device_serial: func_and_params.params[0] })
      );
  ...
  async createDeviceRecord(
    ...
    let drecord = DeviceRecord.createInstance(
      device_serial,
      price,
      manufacturing_date,
      company,
    ...
    // throw new Error() // comment out original code
  ...
  async queryByCompany(ctx, company) {
    ...
    let queryString = {
        ...
        "use_index": "./META-INF/statedb/couchdb/indexes/companyIndex.json"
    };
  ...
  async queryByDevice_Type(ctx, device_type) {
        ...
        let queryString = {
            ...
            "use_index": "./META-INF/statedb/couchdb/indexes/device_typeIndex.json"
        };
        let recordsList = await this.queryWithQueryString(ctx, JSON.stringify(queryString));
        return recordsList;
   }
  ...
  async queryByDevice_Type_Dual(ctx, device_type1, device_type2) {
        ...
        let device_typeList = await this.queryWithQueryString(ctx, JSON.stringify(queryString));
        return device_typeList;
    }
}
